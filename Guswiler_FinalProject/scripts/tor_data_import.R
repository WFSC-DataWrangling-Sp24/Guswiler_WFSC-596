...
# Apply functions to check for sufficient exploration
full_data <- full_data %>%
  mutate(.before = barnes_id,                       #### will choose a better location for this
  ...
  # merge all T/F columns into one, retaining F in any instance.
  #### There is probably a better way to do this that will merge cols and remove the other ones at the same time
  mutate(.before = sample_1_suff_expl,
         sufficient_expl = case_when(sample_1_suff_expl == TRUE &
                                     ...
                                     ~ TRUE,
                                     .default = FALSE)) %>% 
  # remove individual T/F columns, see note above
...
# TO DO:
## determine a better way to merge T/F columns