...
up data by column name (`bind_rows`, adds rows based on column names,
makes data set longer/taller) or row position (`bind_cols`, adds more
columns or variables to the data you already have, males data wider).

![](../../documents/binds.png){width="50%"}
...
all_rodents
#column names need to be exactly the same for any shuffled columns that need to be reorganized to fit data in the correct position.
head(all_rodents)
tail(all_rodents, 20)
```
...
combined
# any row that does not have information in the other data frame will drop.
# good practice to specify join_by() as you can specify two different named columns, but you don't need to include join_by() when the column name you want to join by is exactly the same in both dataframes.
```
...
```{r}
left_join(surveys, species, join_by(species_id))
```
...
```{r}
full_data <- surveys %>% #piping in and using as the first argument, so inner_join knows that surveys is 1st arg
