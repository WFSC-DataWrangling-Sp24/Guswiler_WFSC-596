...
temp_global <- rast("../../documents/image_files/global_temperature.tif")
```
...
temp_sw <- crop(temp_global, southwest_ext)
```
...
```{r pick-colors}
plot(temp_sw,
     col = hcl.colors(n = 50, palette = "YlOrBr"))
```
...
```{r save-plot}
png(filename = "../../outputs/sw-desert-heat.png")
plot(temp_sw, col = temp_cols, 
     main = "Mean Annual Temperature (\u00B0C)")
polys(x = desert_poly)
dev.off()
