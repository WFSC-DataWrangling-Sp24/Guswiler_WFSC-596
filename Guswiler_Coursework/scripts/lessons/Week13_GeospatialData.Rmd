...
```{r terra-load-print}
library(tidyverse)
library(terra) # Won't work with sf today, but there are links in the materials
library(sf)    #  on how to get started with this package
```
...
-   **Raster** data are gridded data. They generally look like images
    and [have a fixed resolution]{.underline}. This means that as you
    zoom in on raster data, you eventually start seeing the grids. Like
    when you zoom in on a photograph and it becomes "pixelated" - we
    will see an example of this later. [Raster data are usually
    quantitative measurements of some sort, such as temperature,
    precipitation, and reflectance in certain light
    wavelengths.]{.underline}
-   **Vector**, or shape, data are defined by specific coordinates on
    the Earth's surface and technically, [do not have a fixed
    resolution.]{.underline} This means that as you zoom in on Vector
    data, it never becomes "pixelated". [Vector data usually take the
    form of points (a single pair of coordinates), lines (two or more
    coordinate pairs connected in a sequence), or polygons (three or
    more coordinate pairs connected in a closed sequence)]{.underline}.
    Note: I apologize at this point for any confusion between the
    definition of geospatial vector data and the vector data structure
    in R. They are not the same thing, even though they probably have
    similar roots.

![Raster data have a fixed resolution and become "pixelated" when
magnifying](../../documents/image_files/zoom-raster.png)

![Vector data do not lose resolution when
magnifying](../../documents/image_files/zoom-vector.png)
...
America](../../documents/image_files/sw-desert-heat.png)
...
global_temp <- rast("../../documents/image_files/global_temperature.tif")
```
...
-   Dimensions: x and y (rows and columns, like a bitmap image), final
    number z is how many layers
-   Extent: we can see it is global because it is the a full 360deg
...
# if you want to use ggplot2 with terra, there is a package you need
#  called "tidyterra"
```
...
with the terra package (via `?terra::plot`, "Make a map").
...
# northwestern-most, southwestern-most, southeasternmost, southwesternmost)
plot(southwest_ext)
```
...
>
> *Answer: Easier to work with and uses the equator and prime meridian
> of earth.*
...
sw_temp <- crop(global_temp, southwest_ext)
```
...
> conversion require?
>
> *Answer: The number of cells, so 403200 times*
...
# Can look at the meta data or the scale on the side to see that we are now in F
```
...
> screen and reality?
>
> *Answer: Distortion of land size*
...
# how our 3D data is transferred into 2D

# EPSG is a unique identifier for the coordinate reference type that you are using!
```
...
```{r reproject-temp}
sw_temp_reproject <- project(sw_temp, "epsg:5070")
plot(sw_temp_reproject)
sw_temp_reproject
```
...
YlOrBr](../../documents/image_files/colorbrewer-palette-name.png)
...
```{r city-data-frame}
cities <- data.frame(city = c("Tucson", "Hermisillo", "Carson City"),
                     lat = c(32.23, 29.02, 39.18),
                     lon = c(-110.95, -110.93, -119.77)) # terra is VERY specific about needing lon NOT long
cities
```

To make these data easier to work with our raster object, we are going
to transform it to a spatial vector, or SpatVector in terra-speak.
[While we are at it, we will tell it to use the same coordinate
reference system as the temperature data we already loaded
in.]{.underline}

```{r cities-vector}
cities_vect <- vect(cities, crs = crs(sw_temp))
```
...
text(cities_vect,
     labels = "city",
     pos = 4,        # pos: position of text relative to the point
     cex = 0.9)      # text size
```
...
prec <- rast("../../documents/image_files/global_precipitation.tif")
...
a desert]{.underline}. We start by defining our cutoff and then creating
a new raster based on values in the precipitation raster.

```{r define-desert}
desert_max <- 250
prec_desert <- prec_sw < desert_max # creates a logical raster (T/F) for if cells are less than 250 mm
