...
```{r}
# download.file("https://ndownloader.figshare.com/files/5629542", "../../data_raw/ACACIA_DREPANOLOBIUM_SURVEY.txt")
# download.file("https://ndownloader.figshare.com/files/5629536", "../../data_raw/TREE.txt")
```
...
```{r}
acacia <- read_tsv("../../data_raw/ACACIA_DREPANOLOBIUM_SURVEY.txt")
```
...
```{r}
# Dead in height category is not ideal, would be better to have dead as it's own column. Replace dead in height as NA.
```
...
```{r}
# na = c("dead", "") <- specifies what should be read in as NAs. As it's written, this will read in blanks and "dead" as NA.
acacia <- read_tsv("../../data_raw/ACACIA_DREPANOLOBIUM_SURVEY.txt"), na = c("dead", "")
```
...
```{r}
ggplot(acacia, mapping = aes(CIRC, HEIGHT)) + # + works similarly to a pipe, but more like a layer rather than, "and then"
  geom_point()
```
...
```{r}
ggplot(acacia, mapping = aes(CIRC, HEIGHT)) +
  geom_point(size = 3, color = "orange", alpha = 0.5)
```
...
ggplot(acacia, mapping = aes(CIRC, HEIGHT)) +
  geom_point(size = 3, color = "orange", alpha = 0.5) +
  labs(x = "Log Circumference (cm)", y = "Log Height (m)") +
  scale_x_log10() +
  scale_y_log10() +
  theme_classic()
```
...
```{r}
ggplot(acacia, mapping = aes(CIRC, HEIGHT, color = TREATMENT)) +
...
`geom_point` uses something called `identity`: the raw form of the data or no transformation. There is no statistical transformation.
...
```{r}
ggplot(acacia, aes(TREATMENT, fill = TREATMENT)) +
  geom_bar() +
...
ggplot(acacia, aes(CIRC)) +
  geom_histogram(binwidth = 5) + # each bin will include 5cm circumference intervals
...
```{r}
ggplot(acacia, aes(CIRC, color = TREATMENT)) +
  geom_histogram(binwidth = 5)

ggplot(acacia, aes(CIRC, fill = TREATMENT)) +
  geom_histogram(binwidth = 5)
```
...
This happens because the default `position` for `geom_histogram` is `"stack"`, which stacks the bars on top of one another and, therefore, makes a stacked histogram.
...
```{r}
ggplot(acacia, aes(CIRC, HEIGHT)) +
  geom_point(size = 3, alpha = 0.5) +
  labs(x = "Circumference (cm)", y = "Height (m)") +
  geom_smooth(method = "lm") +
  theme_classic()

# whichever geom you put first will be layered below whatever geom you specify after
ggplot(acacia, aes(CIRC, HEIGHT)) +
  geom_smooth(method = "lm") +
  geom_point(size = 3, alpha = 0.5) +
  labs(x = "Circumference (cm)", y = "Height (m)") +
...
ggplot(acacia, mapping = aes(CIRC, HEIGHT, color = TREATMENT)) +
  ...
  labs(x = "Circumference (cm)", y = "Height (m)") +
  geom_smooth(method = "lm",
              se = FALSE) +    # remove error margins around lines
  theme_classic()
```
...
```{r}
# color points by treatment, but keep single regression line
ggplot() +
  geom_point(data = acacia, aes(CIRC, HEIGHT, color = TREATMENT)) +
  geom_smooth(method = "lm")  # doesn't show up because we never set it up in ggplot above. Good example how "+" doesn't work like "%>%"
```
...
```{r}
ggplot() +
  geom_point(data = acacia,             # MUST include "data =" here because this is not
             aes(CIRC, HEIGHT,          #  the default order of operations, so not
                 color = TREATMENT)) +  #  assumed.
...
```{r}
# but this is so much better than all the other options. Much faster to type out and cleaner looking.
ggplot(acacia, aes(CIRC, HEIGHT)) +
...
ggsave("../../outputs/acacia_circ_v_height_scatter.png",
       scatterplot,
       height = 5, width = 5)
```
