...
# for each vol. in the vector of volumes, calculate the mass
...
```{r eval = FALSE}
for (i ...) # i represents position in a vector
```
...
```{r eval = FALSE}
for (i in 1:3)  # don't want to have to go back and change the numbers depending on how long vector is
...
-   For a [vector]{.underline}, the `length()` function is useful for this.
-   For a [data frame]{.underline}, the `nrow()` function is often used.
...
```{r}
for (i in 1:length(volumes)) {
  mass <- 2.65 * volumes[i]^0.9 # subsetting a vector for position i
  print(mass)
}
```
...
-   `mode` is the [type of data we are going to store]{.underline}
-   `length` is the [length of the vector]{.underline}

```{r}
masses <- vector(mode = "numeric",
                 length = length(volumes)) # want the new vector to be the same length as our other vector from which we are calculating values
masses
# now we have an empty vector to store our results
```
...
So far, we've talked about looping through one group of values (in a vector). Looping with an index also allows us to access values from multiple vectors. As long as they are in the same position being referenced.
...
masses <- vector("numeric", length(volumes)) # create empty vector

for (i in 1:length(volumes)) {
  mass <- a[i] * volumes[i] ^b[i]
  masses[i] <- mass
}
masses
```
...
est_mass(volumes)
# but you can totally do a vector if you use case_when()
```
...
```{r}
masses <- vector("numeric", length(volumes))

for (i in 1:length(volumes)) {
  mass <- est_mass(volumes[i], a[i], b[i])
  masses[i] <- mass
}
masses
```
...
vol_data <- data.frame(volume = volumes, a = a, b = b)

for (i in vol_data) {
  print(i)
}
#prints each column of the data frame because R default is to index by column
```
...
for (i in 1:nrow(vol_data)) {
  print(vol_data[i,])   # the blank space specifies that we want all columns
}
```
...
# option 1: subset row and column with []

for (i in 1:nrow(vol_data)) {
  mass <- est_mass(vol_data[i, "volume"], #for col can specify the name of the col helpful incase they move to different positions in the data frame
                   vol_data[i, "a"],
                   vol_data[i, "b"])
  masses[i] <- mass
}
...
for (i in 1:nrow(vol_data)) {
  mass <- est_mass(vol_data$volume[i],
                   vol_data$a[i],
                   vol_data$b[i])
  masses[i] <- mass
}
...
```{r}
vol_data$mass <- NA # create new empty col in df

for (i in 1:nrow(vol_data)) {
  mass <- est_mass(vol_data$volume[i],
                   vol_data$a[i],
                   vol_data$b[i])
  vol_data$mass[i] <- mass # puts the answers in the empty col as we would expect
}
vol_data
```
...
unzip("../../data_raw/locations.zip", exdir = "../../data_raw/locations/")
```
...
```{r}
# if you just unzipped them in a general place and not into their own subfolder
list.files("../../data_raw/locations",
           pattern = "locations-")

data_files <- list.files(path = "../../data_raw/locations",
                         pattern = "locations-")
data_files
```
...
results <- vector("numeric", length = num_files)
# or
results <- vector("numeric", length = length(data_files))
```
...
for (i in 1:length(data_files)) {
  ...
  # read in txt file
  data <- read_csv(paste0("../../data_raw/locations/", filename))
  # save number of rows per file
  results[i] <- nrow(data)
}

results

# to double check/troubleshoot your for loop you can set i = a number in list and check that it correctly pulls that specific item in the list and puts it in the correct spot
```
...
```{r}
results_df <- tibble(file_name = vector("character", length(data_files)),
                     count = vector("integer", length(data_files)),
                     min_lat = vector("numeric", length(data_files)))
```
...
for (i in 1:length(data_files)) {
  ...
  results_df$count[i] <- nrow(data)
  results_df$min_lat[i] <- min(data$lat)
}
results_df
```
