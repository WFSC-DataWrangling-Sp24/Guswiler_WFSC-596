...
The shortcut for typing a pipe is `Ctrl` + `Shift` + `M`
...
This works the same way as above; `x` becomes the first argument in the `mean()` function.
...
# mutliple pipes
# recreate intermediate steps above using pipes
surveys %>% 
  filter(species_id == "DS", !is.na(weight)) %>% 
  arrange(year) %>% 
  select(year, weight)

# we need to use the assignment arrow to save the new df
...
[These pipes operate in nearly exactly the same way, and you can use either one you'd like. If you want to change to the base R pipe, you can do so: Tools -\> Global Options -\> Code -\> Use native pipe operator]{.underline}

### Let's Practice

Try your hand at the Portal Data Manipulation Pipes (Exercise 4 and 5).

## [Placeholders]{.underline}
...
This placeholder thing will only work for named arguments.
