...
download.file("https://ndownloader.figshare.com/files/2292172",
  here("data_raw/surveys.csv"))
download.file("https://ndownloader.figshare.com/files/3299474",
  here("data_raw/plots.csv"))
download.file("https://ndownloader.figshare.com/files/3299483",
  here("data_raw/species.csv"))
download.file("https://datacarpentry.org/semester-biology/data/mammal-size-data-clean.tsv",
  here("data_raw/mammal-size-data-clean.txt"))
```
...
avg_rodent_hf_cm <- surveys %>% 
  inner_join(species, join_by(species_id)) %>%   # join surveys with species
  filter(taxa == "Rodent") %>%                   # only pull rodent captures
  mutate(hindfoot_cm = hindfoot_length / 10) %>% # convert mm to cm
  group_by(species_id) %>%                       # want an average for each species
  summarise(mean_hf = mean(hindfoot_cm, na.rm = TRUE)) # calculate mean for groupings
avg_rodent_hf_cm


mean_rodent_hf_cm <- surveys %>% 
...
Develop hypotheses! - See where the code failed. - Read the error
message. - Observe what the code is doing. - Look at the current state
of the environment. This gives you a current snapshot of what is going
on - Talk through the code - [Rubber duck
...
```{r}
do_counts_by_year <- surveys %>% # missing s at end of surveys
  filter(species_id == "DO") %>% # no "species" col. is "species_id, should be ==, not =
  group_by(year) %>%             # missing pipe
  summarize(count = n())
do_counts_by_year

ggplot(do_counts_by_year, aes(x = year, y = count)) + # change from pipe to +, incorrectly spelled data frame
  geom_point() +
  geom_line() +                                       # missing +
  labs(x = "Year", y = "Count") +
  theme_bw()
