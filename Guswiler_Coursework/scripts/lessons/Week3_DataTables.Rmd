...
```{r}
# We won't be using the full tidyverse package for a bit, but we will be using some of the sub packages for now. See these below:
```
...
# You do need "" for installs, but don't need them once you're just pulling from the library
```
...
```{r}
# for the lesson
# download.file("https://ndownloader.figshare.com/files/2292172", here("data_raw/surveys.csv"))
# download.file("https://ndownloader.figshare.com/files/3299474", here("data_raw/plots.csv"))
# download.file("https://ndownloader.figshare.com/files/3299483", here("data_raw/species.csv"))

# for the assignment
# download.file("http://www.datacarpentry.org/semester-biology/data/shrub-volume-data.csv", here("data_raw/shrub-volume-data.csv"))
```

We've already talked about some of the benefits of CSV file. They work very nicely in R.

**Never use the import code button!! You want your code to be runable by anyone that uses it just by running it. If you manually click to import, then that get's left out of the code and won't run the way it should.**
...
We load these into R using a function from the `readr` package called `read_csv()`. `read_csv()` limits the viewer to the first 10 rows, so is less bulky for large data sheets than `read.csv()`

```{r}
# file name in quotes and with file type ending (.csv)

# this converts it into a tibble (a type of data.frame)
...
# dimensions, what the columns are, and what's in those columns
str(species)

# shows the names of columns in the data.frame. Helpful if you can't remember if something is capitalized or not
names(species)

# first 6 rows and columns
head(species)

# Sort of a cleaner structure than what str() shows us.
glimpse(species)
```
...
```{r}
# can pull out specific columns with a function rather than []
...
# you can retrieve col in any order you like
select(surveys, year, month, day)

# you can retrieve based on position as well as column names
select(surveys, month:year)

# remove cols
select(surveys, -weight, -hindfoot_length, -plot_id)
```
...
```{r}
# convert all hindfoot_length values from mm to cm and place into a new col. Always places the new column at the end.
# this alone does not save the new col in our data environment
mutate(surveys, hindfoot_cm = hindfoot_length / 10)
...
```{r}
# need to assign output to an object to save it
surveys_cm <- mutate(surveys, hindfoot_cm = hindfoot_length / 10)

# once we start using multiple of the tidyverse fns into one bigger code chunk. Can't string things together when we use some chunks of base R, they aren't interchangeable.
# so use mutate rather than surveys$hindfoot_cm
```
...
```{r}
# sort table by weight, default is smallest to largest
...
arrange(surveys, plot_id, desc(year), desc(month), desc(day))
```
...
```{r}
# == used when we're writing conditions. "" because it is character data.
...
To get the data on species "DS" for the year greater than 1995:

```{r}
filter(surveys, species_id == "DS", year > 1995)
filter(surveys, species_id == "DS" & year > 1995)

# these arguments to the exact same thing
```
...
```{r}
filter(surveys, species_id == "DS" | year > 1995)
# gives rows in which one or more of these conditions are met

filter(surveys, species_id == "DS" & species_id == "DM")
# pulls up nothing because each row only has one single species_id, so it is impossible for these to be met at the same time, but use the or | function
filter(surveys, species_id == "DS" | species_id == "DM")
```
...
```{r eval=FALSE}
filter(surveys, weight != NA) # doesn't work
filter(surveys, weight != "NA") # it appears to work with "" added?
```
...
# returns all rows with weight == NA
```
