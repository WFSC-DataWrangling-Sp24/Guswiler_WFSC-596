...
Within the CPU, we have cores. Each core is capable of retrieving data and performing tasks.
...
You can't parallelize your code if you need to go from x -\> y -\> z (can't be dependent on a process above or below it). We want to complete tasks on several different features, we break up our process into the different tasks, run them on separate cores, then combine them at the end for our output.
...
```{r}
# you don't have to load a package to pull a function from a package you have installed on your PC
# do this by using package::function
# explicitly calling your functions can be helpful for readibility of your code for other users!
...
```{r}
ratdat_small <- ratdat[1:1000, ] # 1000 rows, but all columns
```
...
```{r}
tags <- unique(ratdat_small$tag) # each unique value in tag column
...
# we see that we have the same number of rows in our df as # of unique tags
# success!
# as we add more rows in our ratdat, it will take longer and longer
```
...
```{r}
ratdat_medium <- ratdat[1:10000, ]

tags <- unique(ratdat_medium$tag)


capt_histories <- data.frame()

for (i in 1:length(tags)) {
  output <- create_capture_hist(tags[i], ratdat_medium)
  capt_histories <- rbind(capt_histories, output)
}
```
...
```{r}
capt_histories <- lapply(tags, create_capture_hist, data = ratdat_medium)
```

We will get a list returned to us. To convert the results into a dataframe, we can use the `do.call` function to treat each list as a row and bind them together.
...
```{r}
parallel::detectCores() # You've got 8 cores :P
nCores <- detectCores()/2
# good practice to at minimum leave 1 to 2 cores for your PC to continue normal functions. Elen does half their cores.
```

We can now use the `parLapply` function just as we did with `lapply` but with the added `mc.cores` argument. Before hand, to make this work we need to make a cluster: grouping together all the cores that you want to use into an entity.

```{r eval=FALSE}
clust <- makeCluster(nCores)
# output is a group of the cores you're using. Basically a baby network of cores.


results <- parLapply(cl = clust, tags,
                     create_capture_hist,
                     data = ratdat_medium)
```
...
```{r eval=FALSE}
output <- foreach::foreach(i = 1:length(tags)) %do% {
  create_capture_hist(tags[i], ratdat_small)
}
# the output from here is a large list of tiny data frames. We can use .combine to call rbind to make it into a full data frame.

output <- foreach(i = 1:length(tags), .combine = rbind) %do% {
  create_capture_hist(tags[i], ratdat_small)
}
# now our output looks like a data frame!
```
...
# you want to stop your cluster after completing the parallelized portion of your code.
```

## High Performance Computing at UA

If you are working with data that would benefit from parallelization (and especially parallelization on a large scale), [consider learning about the High Performance Computing that UA offers]{.underline}.

[HPC access is available to anyone affiliated with UA, with a certain number of computing hours offered for free each month]{.underline}. You will likely need to be linked to a research group (AKA your PI's account).
