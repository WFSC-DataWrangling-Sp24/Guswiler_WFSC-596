...
```{r}
detectCores() # You've got 8 cores :P
nCores <- detectCores()/2
# good practice to at minimum leave 1 core for your PC to continue normal functions. Elen does half their cores


capt_histories <- mclapply()
  lapply(tags, create_capture_hist, data = ratdat_medium)
```

We can now use the `mclapply` function just as we did with `lapply` but with the added `mc.cores` argument.

```{r eval=FALSE}
capt_histories <- mclapply(tags, create_capture_hist, data = ratdat_medium, mc.cores = 4)
```
