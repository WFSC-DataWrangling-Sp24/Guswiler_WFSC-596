...
Our ultimate goal is to be able to rerun a full analysis with a single click (or command).
...
-   Run entire file by selecting "Run All" in RMarkdown (`Ctrl + Alt + R`)
...
-   If you don't want to remove them, comment them out by putting a `#` in front of the line of code.
    -   A shortcut for this is `Ctrl + Shift + C`
...
# doesn't work, like it shouldn't
```
...
For example, on a Windows machine, the directory is `C:/Users/username`. Within the `username` directory is the `Downloads` directory (or folder).

```{r eval = FALSE}
# Absolute file path to file in downloads
data <- read_csv('C:/Users/Olivia/Downloads/shrub-dimensions-labeled.csv')
# DON'T DO THIS, WE WANT RELATIVE PATHS
```
...
If my current location is in `C:/Users/Olivia`, then the absolute and relative paths here are the same.
...
# R Markdown (.Rmd) files work differently than script files and automatically create working directory in the environment that you open the file in. 
```
...
```{r eval = FALSE}
data <- read_csv(here('data_raw/shrub-dimensions-labeled.csv'))
```
...
Once we create our new project, RStudio will create a `.Rproj` file in that directory. This file is not the project itself, but it does contain information about the project. [We do not manually change this file.]{.underline}
...
```{r eval = FALSE}
# if it's stored in the working directory
data <- read_csv("shrub-dimensions-labeled.csv")
```
...
# want to save this to work with in the future
write_csv(shrub_data, here('data_clean/shrub_volume.csv'))

# now can pull this file in from data_clean
shrub_data <- read_csv(here('data_clean/shrub_volume.csv'))
```
...
-   data_clean
-   docs
    -   writing that you're working on (i.e. sections or manuscript)
-   outputs
...
[The R Project folder should also contain a README file with descriptions of the file structure, the files in the project, and any other relevant information for how to reproduce analyses or outputs.]{.underline}
...
```{r eval = FALSE}
# because the .Rmd is in /scripts, it makes this the working directory.
getwd()  # "C:/Users/Olivia/Documents/WFSC-596/WFSC-596_R/scripts"

data <- read_csv('../data_raw/shrub-dimensions-labeled.csv')

# can also use TAB after typing part of the file structure "../" or "data_raw/" and it will show you the file directory for you to visually look through and select to auto-fill in.
### VERY USEFUL ###
```
...
It is easiest to start a new project when you create a repo, but it is possible to put a current proj in a new repo, just a lot more steps.
