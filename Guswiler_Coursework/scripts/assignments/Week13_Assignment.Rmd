---
title: 'Assignment 13'
author: "Olivia Guswiler"
date: "`r Sys.Date()`"
output: pdf_document
editor_options: 
  markdown: 
    wrap: 72
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

# Assignment Details

### Purpose

The goal of this assignment is to practice working with geospatial
raster and vector data.

### Task

Write R code to successfully answer each question below.

### Criteria for Success

-   Code is within the provided code chunks or new code chunks are
    created where necessary
-   Code chunks run without errors
-   Code chunks have brief comments indicating which code is answering
    which part of the question
-   Code will be assessed as follows:
    -   Produces the correct answer using the requested approach: 100%
    -   Generally uses the right approach, but a minor mistake results
        in an incorrect answer: 90%
    -   Attempts to solve the problem and makes some progress using the
        core concept, but returns the wrong answer and does not
        demonstrate comfort with the core concept: 50%
    -   Answer demonstrates a lack of understanding of the core concept:
        0%
-   Any questions requiring written answers are answered with sufficient
    detail

### Due Date

**This Assignment is Optional!** April 22 at midnight MST

# Assignment Exercises

### 1. Working with raster data in terra (20 pts)

In this exercise, you will work with the same precipitation data as in
class, but this time focus in on the Himalayan region of Asia.

a.  Load in the terra package. What version does it say you are using?

b.  Load in the precipitation data ("global_precipitation.tif"), what is
    the EPSG used in these data?

c.  Define a geographic extent object for the Himalayan region.

d.  Crop the precipitation data to the Himalayan extent; what are
    resulting dimensions?

```{r}
#1a
library(terra)
# terra 1.7.71

#1b
precip_global <- rast("../../documents/image_files/global_precipitation.tif")
precip_global
# ESPG: 4326

#1c
himalaya_ext <- ext(c(69, 97, 26, 40))

#1d
precip_himalaya <- crop(precip_global, himalaya_ext)
precip_himalaya
# nrow = 336, ncol = 672, nlyr = 1
```

### 2. Printing maps (20 points)

Still working with the precipitation data, this exercise focuses on
visualizing the data.

a.  Plot the precipitation data for the Himalayan region.

b.  Add a title to the plot.

c.  Change the palette to that of `topo.colors` (use `?topo.colors` for
    details) and print the resultant plot.

d.  Change the palette to one from
    [ColorBrewer](https://colorbrewer2.org) for sequential (continuous)
    data.

```{r}
#2a
plot(precip_himalaya)

#2b
plot(precip_himalaya,
     main = "Mean Annual Precipitation (mm)")

#2c
plot(precip_himalaya,
     main = "Mean Annual Precipitation (mm)",
     col = topo.colors(n = 50))

#2d
plot(precip_himalaya,
     main = "Mean Annual Precipitation (mm)",
     col = rev(hcl.colors(n = 50, palette = "YlGnBu")))
```

### 3. Working with vector data in terra (20 points)

a.  Create a data frame of three peaks in the Himalayan region: (lat,
    lon)

-   K2: 35.93, 76.50
-   Annapurna: 28.53, 83.81
-   Mount Everest: 28.05, 86.91

b.  Create a SpatVector object from the data frame with peak data.

c.  Convert the points SpatVector object to a lines object.

d.  Add the lines object to the precipitation plot of the Himalayan
    region.

```{r}
#3a
peaks <- data.frame(peak = c("K2", "Annapurna", "Mount Everest"),
                    lat = c(35.93, 28.53, 28.05),
                    lon = c(76.50, 83.81, 86.91))
peaks

#3b
peaks_vect <- vect(peaks, crs = crs(precip_himalaya))
peaks_vect

#3c
peaks_line <- as.lines(peaks_vect)
peaks_line

#3d
plot(precip_himalaya,
     main = "Mean Annual Precipitation (mm)",
     col = rev(hcl.colors(n = 50, palette = "YlGnBu")))
plot(peaks_line, add = TRUE)
```

### 4. Modifying raster values (20 points)

a.  Load in the elevation data ("global_elevation.tif"), what are the
    units for these data?

b.  Convert the units of the raster to feet.

c.  Crop the elevation data to the same geographic extent as
    precipitation data.

d.  Plot the Himalayan elevation data.

```{r}
#4a
elev_global <- rast("../../documents/image_files/global_elevation.tif")
elev_global
# meters

#4b
elev_global_ft <- elev_global * 3.28084
elev_global_ft

#4c
elev_himalaya <- crop(elev_global, himalaya_ext)
elev_himalaya

#4d
plot(elev_himalaya,
     main = "Elevation (m)")
```

### 5. Converting raster objects to spatial vector objects (20 points)

For this last exercise, create a plot that shows precipitation for the
Himalayan region, using a color palette from ColorBrewer AND add a layer
that is partially (50%) transparent to shade out regions below 13,000
feet.

```{r}
# crop elev (ft) to himalaya region
himalaya_ft <- crop(elev_global_ft, himalaya_ext)

# apply logic: is cell value less than 13000 ft
himalaya_13000ft_min <- himalaya_ft < 13000 

# false values changed to NA
himalaya_13000ft_min[!himalaya_13000ft_min] <- NA

# make a polygon from logical himalaya raster
himalaya_poly_ft <- as.polygons(himalaya_13000ft_min)


plot(precip_himalaya,    # plot precip data
     main = "Mean Annual Precipitation (mm)",
     col = rev(hcl.colors(n = 50,
                          palette = "YlGnBu")))
polys(himalaya_poly_ft,     # add polygon to plot
      col = "black",        # change fill color to black
      alpha = 0.5,          # set transparency to 50%
      border = FALSE)       # remove polygon outline

```
