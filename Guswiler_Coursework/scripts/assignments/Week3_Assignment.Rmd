...
```{r, message = FALSE}
# load libraries, download csv for the assignment, and read .csv
library(dplyr)
library(readr)
library(here)
...
shrub <- read_csv(here("data_raw/shrub-volume-data.csv"))
```
...
#1c
mutate(shrub, area = length * width)

#1d
arrange(shrub, length)

#1e
filter(shrub, height > 5)

#1f
filter(shrub, height > 4 & width > 2)

#1g
filter(shrub, experiment == 1 | experiment == 3)

#1h
filter(shrub, !is.na(height))

#1i
shrub_volumes <- mutate(shrub, volume = length * width * height)
shrub_volumes
```
...
```{r}
#2
# import tucson precip data
ppt_data <- read.csv(here("data_raw/tucson_precip.csv"), header = FALSE)

# calculate mean ppt for each month and assign list to a variable
monthly_mean_ppt <- colMeans(ppt_data)

# line plot of the monthly average precip in Tucson from 2000 to 2023
...
#3b
surveys_kg <- mutate(surveys, weight_kg = weight / 1000) # convert mass from g to kg
surveys_wt.na <- filter(surveys_kg, !is.na(weight))      # remove entries with weight == "NA"
surveys_yr_id_wt <- select(surveys_wt.na, year, 
                           species_id, weight_kg)        # new data frame with year, species id, 
surveys_yr_id_wt                                         #   and weights in kg (weight == "NA" removed)
...
```{r}
#4a
surveys_pipe.a <- surveys |>           # define the group being trasformed
  filter(!is.na(weight)) |>            # remove weight == "NA"
  mutate(weight_kg = weight/1000) |>   # convert weights g to kg
  select(year, species_id, weight_kg)  # cols included in new data.frame
surveys_pipe.a

#4b
surveys |>                                # define group being transformed
  select(year, month, day, species_id) |> # select desired cols
  filter(species_id == "SH")              # return only rows with species "SH"
```
...
#5
surveys |>                                 # define group being transformed
  filter(species_id == "DM" |              # filter to include only Dipodomys
           species_id == "DO" | 
           species_id == "DS",
         !is.na(weight),                   # also filter out NA in weight and hindfoot
         !is.na(hindfoot_length)) |> 
  select(year, month, day, species_id,     # define desired columns for output
         weight, hindfoot_length) |> 
  arrange(species_id, desc(weight)) |>     # sort table by species and weight (greatest to smallest)
  mutate(species_id = tolower(species_id)) # change species IDs to lower case
```
