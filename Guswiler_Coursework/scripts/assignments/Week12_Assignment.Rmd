...
for (number in numbers){  # specify the object and the vector containing object
  product <- number * 3   # what will be done to each object in the vector
  print(product)          # print the product of each object
}
```
...
```{r}
dinosaur_lengths <- read_csv("../../data_raw/dinosaur_lengths.csv") # read in file

# create function to calculate masses
mass_from_length <- function(length, species) {
  species <- tolower(species)
  mass <- case_when(species == "stegosauria" 
                    ~ 10.95 * length ^ 2.64,
                    species == "theropoda" 
                    ~ 0.73 * length ^ 3.63,
                    species == "sauropoda"
                    ~ 214.44 * length ^ 1.46,
                    .default = 25.37 * length ^ 2.49)
  return(mass)
}

#2a
dinosaur_masses <- vector("numeric",   # create a new vector to hold numeric output
                        length(dinosaur_lengths$lengths)) # same L as dino_lengths

for (i in 1:length(dinosaur_lengths$lengths)) {
  mass <- mass_from_length(dinosaur_lengths$lengths[i],
                           dinosaur_lengths$species[i])
  dinosaur_masses[i] <- mass
}
head(dinosaur_masses)

#2b
dinosaur_lengths <- dinosaur_lengths %>% # add masses to df
  mutate(masses = dinosaur_masses)
...
download.file("http://www.datacarpentry.org/semester-biology/data/individual_collar_data.zip",
              destfile = "../../data_raw/individual_collar_data.zip")
unzip("../../data_raw/individual_collar_data.zip",
      exdir = "../../data_raw/individual_collar_data")

# make list of file names
collar_file_names <- list.files("../../data_raw/individual_collar_data/",
                                "collar-data-.*.txt")

#3a
for (i in 1:length(collar_file_names)) {
  # read in the file
  collar_data <- read_csv(show_col_types = FALSE,
                          paste0("../../data_raw/individual_collar_data/",
                                 collar_file_names[i]))
  # plot long and lat values for each file
  print(ggplot(collar_data, aes(long, lat)) +
          geom_line() +
          labs(title = paste0("Location Tracking of Collar ", collar_data$collar),
               x = "Longitude",
               y = "Latitude"))
}
...
collar_df <- tibble(file_name = vector("character", length(collar_file_names)),
                    min_lat = vector("numeric", length(collar_file_names)),
                    max_lat = vector("numeric", length(collar_file_names)))

for (i in 1:length(collar_file_names)) {
  # read in the file
  collar_data <- read_csv(paste0("../../data_raw/individual_collar_data/",
                          collar_file_names[i]))
  # add info to the empty data frame
  collar_df$file_name[i] <- collar_file_names[i]
  collar_df$min_lat[i] <- max(collar_data$lat)
  collar_df$max_lat[i] <- min(collar_data$lat)
}
collar_df
```
...
1.  What has worked well for you in this course for you so far?
    ...
    *Answer: The flexibility with assignment due dates and providing information on the final project so early in the semester. It allowed me to get a head start on the project and made this busy time of year a bit less stressful.*

2.  What has been particularly challenging for you so far?

    *Answer: The for loops tripped me up a bit, particularly when to reference [i] in the loop.*

3.  Is there anything that I can do to help your learning in the course?

    *Answer: Nope, you've been doing great!*

4.  Other reflections about the content of the course that you would like to share.

    *Answer: It was nice having Jeff in go over how to work with spatial data, even though we didn't get through everything during the lecture, it was very helpful and informative.*
