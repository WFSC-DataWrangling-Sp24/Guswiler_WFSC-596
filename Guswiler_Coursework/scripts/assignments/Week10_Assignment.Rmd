...
```{r}
#1a
convert_pounds_to_grams <- function(pounds) {  # name fn and assign input
    grams <- 453.6 * pounds                    # script for what fn will perform
    return(grams)                              # assign return object
}
...
#2b
get_mass_from_length <- function(a, b, length) { # name fn and assign input
  mass <- a * length ^ b                         # script for what fn will perform
  return(mass)                                   # assign return object
}
get_mass_from_length(214.44, 1.46, 26)
```

### 3. Writing Functions, 2 (15 points)
...
convert_pounds_to_grams <- function(pounds,
                                    numdigits = 1) { # default to 1 dec. place
  grams <- 453.6 * pounds
  rounded <- round(grams, digits = numdigits)
  return(rounded)
}
...
#3b
get_height_from_weight <- function(weight_g, a = 12, b = 0.38) {
  height <- a * weight_g ^ b
  return(height)
}
...
#3c
get_height_from_weight <- function(weight_lb, a = 12, b = 0.32) {
  weight_g <- convert_pounds_to_grams(weight_lb)
  height <- a * weight_g ^ b
  return(height)
}
get_height_from_weight(2)
```
...
get_mass_from_length <- function(length,
                                 a = 39.9, b = 2.6) { # default values for objects
  mass <- a * length ^ b
  return(mass)
}
...
get_mass_from_length(10.95, 2.64, 12) %>% 
...
#6a
count_data <- tibble(state = state_vector, # create data frame from vectors
                     count = count_vector,
                     area = area_vector,
                     site = site_vector)
count_data
...
#6b
make_scatter <- function(df, col_area, col_count, color_by) {
  ggplot(df, aes({{col_area}}, {{col_count}})) +       # plotted data and mapping
    geom_point(size = 3, aes(color = {{color_by}})) +  # plot type and color
    labs(x = "Area (ha)", y = "Number of Individuals") # axes labels
}
make_scatter(count_data, area, count, state)

ggsave("../../outputs/Assignment10_6b_plot.png") # save most recent plot as .png

#6c
make_scatter(count_data, area, count, site)

ggsave("../../outputs/Assignment10_6c_plot.png")
```
