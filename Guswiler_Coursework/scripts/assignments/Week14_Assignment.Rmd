...
Let's practice using `lapply` and the mutli-core cousin, `parLapply`.
...
Use the `head` function to look at the `seq_list` object. You will notice that the output looks different from the vectors and dataframes that we are used to looking at. That is because it is a list, a collection of data objects. But don't worry! The code for `lapply` and `parLapply` is the same as in the lesson.
...
```{r}
#2a
sqrt_list <- seq_list %>%            # take sqrt of list items
  lapply(sqrt)

sqrt_df <- do.call(rbind, sqrt_list) # bind list items into data frame

head(sqrt_df)

#2b
nCores <- detectCores() - 2  # set number of cores to utilize in parallel

clust <- makeCluster(nCores) # group cores into network

sqrt_list <- parLapply(clust, seq_list, sqrt) # use multiple cores to take sqrt of list

sqrt_df <- do.call(rbind, sqrt_list)
head(sqrt_df)
```
...
#3a
dino_masses <- vector("numeric",   # create a new vector to hold numeric output
                        length(dino_lengths$lengths)) # same L as dino_lengths

for (i in 1:length(dino_lengths$lengths)) {
  mass <- mass_from_length(dino_lengths$lengths[i],
                           dino_lengths$species[i])
  dino_masses[i] <- mass
}
head(dino_masses)

#3b
dino_masses <- foreach(i = 1:length(dino_lengths$lengths), .combine = c) %do% {
  mass_from_length(dino_lengths$lengths[i],
                   dino_lengths$species[i])
}
head(dino_masses)

#3c
registerDoParallel(nCores) # register cores into a cluster

dino_masses <- foreach(i = 1:length(dino_lengths$lengths), .combine = c) %dopar% {
  mass_from_length(dino_lengths$lengths[i],
                   dino_lengths$species[i])
}
head(dino_masses)

stopImplicitCluster()      # remove cores from cluster
...
```{r}
forest <- read_csv("../../data_raw/forest_per_country.csv", 
...
(a) Write a for loop that calculates the change in forest per country from 1990 to 2021 by subtracting the 1990 value from the 2021 value. The output should be stored in a vector called `change_in_forest`. Print the head of `change_in_forest`.
...
```{r}
#4a
change_in_forest <- vector("numeric", nrow(forest)) # create empty vector

for (i in 1:nrow(forest)) {
  diff <- forest$'2021'[i] - forest$'1990'[i] # take diff btwn forest in 1990 and 2021
  change_in_forest[i] <- diff                 # store diff values in empty vector
}
head(change_in_forest)

#4b
change_in_forest <- foreach(i = 1:nrow(forest), .combine = c) %do% {
...
change_in_forest <- foreach(i = 1:nrow(forest), .combine = c) %dopar% {
  forest$'2021'[i] - forest$'1990'[i]
}
head(change_in_forest)
