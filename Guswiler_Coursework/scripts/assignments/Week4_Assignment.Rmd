...
# upload .csv data to environment
...
surveys %>% 
  ...
  summarise(avg_mass = mean(weight, na.rm = TRUE))
```
...
```{r, include=FALSE}
shrub_dims <- read_csv(here('data_raw/shrub-volume-data.csv'))
shrub_dims %>% 
  group_by(site) %>% 
...
Add a line of code to use `max()` to determine the maximum height of a plant in each experiment.
...
```{r}
#4a
inner_join(surveys, species, join_by(species_id))
...
surveys %>% 
  inner_join(plots, join_by(plot_id)) %>% 
  inner_join(species, join_by(species_id)) %>%          # join dataframes
  filter(taxa == "Rodent",                              # filter for relevant obs
         !is.na(weight),
         plot_type == "Control" |
           plot_type == "Long-term Krat Exclosure") %>% 
  select(year, genus, species, weight, plot_type)       # select desired variables
```

### 6. Shrub Volume Bind (10 pts)

First, run the following code chunk to produce a data frame with additional data related to the shrub volume data (`shrub_dims`).
...
*Answer: The data should be bound by* `bind_cols()` *because the different variables for the same observations are defined by columns.*
...
Use `inner_join()` to combine `experiments` with the `shrub_dims` data to add a manipulation column to the shrub data.
...
```{r}
#7
shrub_data <- shrub_dims %>% 
  inner_join(experiments, join_by(experiment)) %>% # combine experiments and shrub_dim
  inner_join(sites, join_by(site))                 # combine with sites
shrub_data
```
...
```{r}
#9
data.frame(length, width, height,    # combine vectors
           genus = "Cercidium",      # add columns for genus and species
